### Usage: python covpergenome_from_covpercontig.py <accession> <coverage.percontig.perGBP.tsv> <genomename>

from sys import argv

with open(argv[2], 'r') as covpercont:
...
for line in data:
    if line != '':
        if line.split('\t')[0].startswith(argv[3]):
...
print(Accession + '\t' + str(genomelen) + '\t' + str(GCxlen/genomelen) + '\t' + str(mapped_bases) + '\t' + str(covPERCxlen/genomelen) + '\t' + str(avgCOVxlen/genomelen))
